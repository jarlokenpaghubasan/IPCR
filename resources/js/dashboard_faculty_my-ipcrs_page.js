        const myIpcrsConfigEl = document.getElementById('faculty-my-ipcrs-page-config');
        let myIpcrsConfig = {};

        if (myIpcrsConfigEl) {
            try {
                myIpcrsConfig = JSON.parse(myIpcrsConfigEl.textContent || '{}');
            } catch (error) {
                myIpcrsConfig = {};
            }
        }

        const myIpcrsRoutes = myIpcrsConfig.routes || {};
        const ipcrRoleLabel = myIpcrsConfig.ipcrRoleLabel || 'Faculty';
        const csrfToken = myIpcrsConfig.csrfToken || '';
        const isDeanRole = !!myIpcrsConfig.isDean;
        ...
        async function importXlsxFile(file, docType) {
            ...
            try {
                const response = await fetch(myIpcrsRoutes.ipcrImport || '/faculty/ipcr/import', {
                    method: 'POST',
                    headers: {
        ...
        window.saveIpcrDocument = function() {
            ...
            const url = currentSavedCopyId
                ? `${myIpcrsRoutes.savedCopiesBase || '/faculty/ipcr/saved-copies'}/${currentSavedCopyId}`
                : (myIpcrsRoutes.savedCopiesStore || '/faculty/ipcr/saved-copies');
        ...
        window.saveAsTemplate = function() {
            ...
            fetch(myIpcrsRoutes.templatesFromSavedCopy || '/faculty/ipcr/templates/from-saved-copy', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrfToken,
        ...
        async function getSavedCopies() {
            try {
                const response = await fetch(myIpcrsRoutes.savedCopiesIndex || '/faculty/ipcr/saved-copies', {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrfToken,
        ...
        window.deleteSavedCopy = function(id) {
            openConfirmationModal(
                ...
                'Delete',
                function() {
                    fetch(`${myIpcrsRoutes.savedCopiesBase || '/faculty/ipcr/saved-copies'}/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
        ...
        window.generateIPCR = function() {
            ...
            const url = isEditMode
                ? `${myIpcrsRoutes.templatesBase || '/faculty/ipcr/templates'}/${currentTemplateId}`
                : (myIpcrsRoutes.ipcrStore || '/faculty/ipcr');
        ...
        window.loadTemplateToDocument = function(templateId) {
            fetch(`${myIpcrsRoutes.templatesBase || '/faculty/ipcr/templates'}/${templateId}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrfToken,
        ...
        function setActiveTemplate(templateId) {
            fetch(`${myIpcrsRoutes.templatesBase || '/faculty/ipcr/templates'}/${templateId}/set-active`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrfToken,
        ...
        function setActiveSubmission(submissionId) {
            fetch(`${myIpcrsRoutes.submissionsBase || '/faculty/ipcr/submissions'}/${submissionId}/set-active`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrfToken,
        ...
        function useTemplateAsDraft() {
            ...
            const endpoint = docTypeToUse === 'opcr' 
                ...
                : `${myIpcrsRoutes.templatesBase || '/faculty/ipcr/templates'}/${templateId}/save-copy`;
        ...
        function saveCopyFromTemplate(templateId) {
            fetch(`${myIpcrsRoutes.templatesBase || '/faculty/ipcr/templates'}/${templateId}/save-copy`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': csrfToken,
        ...
        function updateSubmissionData(submissionId) {
            ...
            const baseUrl = submissionType === 'opcr'
                ? '/faculty/opcr/submissions'
                : (myIpcrsRoutes.submissionsBase || '/faculty/ipcr/submissions');
        ...
        window.viewSubmission = function(submissionId) {
            ...
            fetch(`${myIpcrsRoutes.submissionsBase || '/faculty/ipcr/submissions'}/${submissionId}?t=${Date.now()}`, {
                headers: {
        ...
        function updateSubmission(submissionId) {
            ...
            fetch(`${myIpcrsRoutes.submissionsBase || '/faculty/ipcr/submissions'}/${submissionId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
        ...
        function fetchSoDocCount(soLabel, countElement) {
            ...
            fetch(`${myIpcrsRoutes.supportingDocumentsIndex || '/faculty/supporting-documents'}?documentable_type=${docType}&documentable_id=${docId}&so_label=${encodeURIComponent(soLabel)}${ownerParam}`, {
        ...
        function loadSoDocuments() {
            ...
            fetch(`${myIpcrsRoutes.supportingDocumentsIndex || '/faculty/supporting-documents'}?documentable_type=${type}&documentable_id=${id}&so_label=${encodeURIComponent(label)}${ownerParam}`, {
                headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': csrfToken }
            })
            .then(r => r.json())
            .then(data => {
        ...
        window.uploadSoDocument = function() {
            ...
            xhr.open('POST', myIpcrsRoutes.supportingDocumentsStore || '/faculty/supporting-documents');
